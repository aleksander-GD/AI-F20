...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        # rowResult = self.probability_table
        sumRow = 0
        sumColumn = 0
        sumColumn2 = 0
        SumOffirstColomun = 0
        ...
        for key, value in self.probability_table.items():
            propability_values = value
            probability_tuple_list.append(value)
            # add f.eks. (0.5, 0,5)
            for row_entry in propability_values:  # Det her er for rows ikke for kolonner
                sumRow += row_entry
                # Plus hvert row entry f.eks. (0.5, 0.5) giver 1.0, dette gælder selvfølgelig i alle tilfælde hvis man kigger på slide 1 i exercise
            n += sumRow
            if len(probability_tuple_list) > 1 and not self.name.lower() == 'wetgrass' or (
                    len(probability_tuple_list) == 4 and self.name.lower() == 'wetgrass'):
                sumColumn = sum([pair[0] for pair in
                                 probability_tuple_list])  # for hvert tuple element Summere første element i tuplen i listen
                sumColumn2 = sum([pair[1] for pair in probability_tuple_list])  # samme som sumColumn2

            if not self.parents:
                sumColumn = value[0]
                sumColumn2 = value[1]

            n += sumColumn2 + sumColumn

            sumRow = 0  # Reset sumRow
        print('Total probability value of N: ', n, ' for node: ', self.name)
        self.marginal_probabilities = [sumColumn / n, sumColumn2 / n]

        # self.marginal_probabilities = [sumColumn, sumColumn2]
